  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0" aria-hidden>
        ...
        {/* Unified semi-transparent overlay to ensure maximum contrast for white text */}
        <div className="absolute inset-0 bg-black/45 z-1" />
      </div>

      <Container size="hero" className="relative z-10 py-24 md:py-28 w-full">
        {/* Center-aligned layout container */}
        <div className="mx-auto max-w-4xl text-center flex flex-col items-center">
          
          {/* Glowing Eyebrow Status */}
          {heroEyebrow ? (
            <div className="inline-flex items-center justify-center gap-2.5 mb-4 !text-[#f0a14a] font-bold text-xs uppercase tracking-[0.25em]">
          ...
          {/* Typography Headline (Forced White Color) */}
          <motion.h1
          ...
          {/* Subtitle Description (Forced White/88 Color) */}
          {heroSubtitle ? (
            <motion.p
              initial={{ opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ ...defaultTransition, delay: 0.14 }}
              className="mx-auto mt-6 !text-white/88 max-w-2xl font-light text-base md:text-lg leading-relaxed text-balance"
            >
              {heroSubtitle}
            </motion.p>
          ) : null}
          ...
          {pillars.length > 0 ? (
            ...
            >
              <div className="inline-flex flex-wrap items-center justify-center gap-4 rounded-full bg-white/10 backdrop-blur-md px-5 py-2.5 border border-white/10 shadow-lg">
                {pillars.map((pill, i) => (
          ...
          <motion.div
            ...
            className="mx-auto mt-10 flex w-full max-w-[22rem] flex-col items-stretch gap-4 sm:max-w-none sm:flex-row sm:items-center sm:justify-center sm:gap-4"
          >
            {primaryLabel ? (
              <WhatsAppButton
